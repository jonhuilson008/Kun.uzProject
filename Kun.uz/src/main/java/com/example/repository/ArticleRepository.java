...
@Query("update ArticleEntity set status = ?1, title = ?2,description = ?3," +
    ...
    int update(ArticleStatusEnum status,
               String title,
               String description,
               String content,
               Integer regionId,
    ...
    int status(String id, ArticleStatusEnum status);
...
*/
    List<ArticleEntity> getTop8ByStatusAndIdNotInOrderByCreatedDate(ArticleStatusEnum status, List<String> idList);

    Optional<ArticleEntity> findByIdAndStatusAndVisibleTrue(String id, ArticleStatusEnum status);
